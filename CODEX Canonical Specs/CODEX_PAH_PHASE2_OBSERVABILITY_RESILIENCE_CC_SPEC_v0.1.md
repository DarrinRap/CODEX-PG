# CODEX PAH Phase 2 Observability and Resilience Spec v0.1
...
Add or extend a structured PAH health-details payload that can be used by tray status, local scripts, and support bundles. It may be a new endpoint or an extension of an existing local endpoint, but it must remain local-only and backward compatible with existing endpoint consumers.
...
- Retain enough recent history for diagnosis while avoiding unbounded file growth; default retention should be either rotation at approximately 5 MB or the most recent 1,000 events, whichever is easier to implement consistently with existing PAH log patterns.
